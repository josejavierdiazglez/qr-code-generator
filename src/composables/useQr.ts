import type { StateSchema } from '@/state/url-state'
import type { DotStyle, EcLevel, EyeBallStyle, EyeFrameStyle, QrInput, QrMatrix } from '@/qr'
import { computed, ref, shallowRef, watchEffect } from 'vue'
import { buildPayload, encodeQr, renderSvg } from '@/qr'
import { svgToPngBlob } from '@/qr/render-canvas'
...
export function useQr() {
  const state = useUrlState(qrStateSchema)
  const logoHref = ref('')
  ...
  function reset() {
    Object.assign(state, defaultState(qrStateSchema))
    logoHref.value = ''
  }

  return { state, logoHref, payload, matrix, svg, error, toPngBlob, reset }
}
